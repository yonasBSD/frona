...
required:
  ...
  - instruction
---
Create a task — one-off or recurring, for yourself or another agent. Do the work yourself when you have the tools and capability. Only create tasks to: defer work to a later time, run background work in a separate context, parallelize across multiple agents, or leverage another agent's specialized capabilities (listed in <available_agents>). Omit target_agent to create a task for yourself. Set cron_expression for recurring work. Set process_result to receive and act on the result yourself; omit it to let the result post directly to the chat. For periodic autonomous check-ins, use set_heartbeat.
