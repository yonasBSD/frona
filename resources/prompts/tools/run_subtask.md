...
required:
  ...
  - instruction
---
Run a subtask on another agent and resume when it completes. Unlike delegate_task, the result is returned to YOU (the calling agent) so you can process it further. Only use this when you must transform, combine, or act on the sub-agent's output to complete your own task. If the user can consume the result directly, use delegate_task instead. Optionally set run_at to defer execution. For relative delays, use shell first: `date -d "+2 minutes" +%s` to get the timestamp. Omit timing language from instruction — just describe the action.
